...
Obsidian이라면 커맨드 팔레트(`Ctrl/Cmd+P`) → **"Templater: Insert 온보딩"** 실행 — 몇 가지 질문에
답하면 `profile.md`/`case-index.md`가 자동 생성된다 (`_templates/온보딩.md`). **몇 번을 다시
실행해도 안전** — 매번 전체 항목을 다시 물어보되 이미 있는 값이 기본값으로 채워져 있어서,
그냥 엔터만 치면 유지되고 고칠 게 있으면 그 자리에서 수정하면 된다. 매수 이력을 추가할 땐
